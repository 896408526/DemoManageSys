...
namespace BLL
{
    public class MenuInfoBLL : IMenuInfoBLL
    {
        ...
        {
            var data = from m in _menuInfoDAL.GetEntities().Where(m => m.IsDelete == false)
                       join m2 in _menuInfoDAL.GetEntities().Where(m => m.IsDelete == false)
        ...
        {
            var menuData = _menuInfoDAL.GetEntities().Where(it => it.IsDelete == false).Select(d => new
            {
                value = d.Id,
                title = d.Title,
            });
