...
namespace BLL
{
    public class DepartmentInfoBLL : IDepartmentInfoBLL
    {
        ...
        {
            var data = from d in _departmentInfoDAL.GetEntities().Where(d => d.IsDelete == false)
                       join u in _userInfoDAL.GetEntities().Where(u => u.IsDelete == false)
                       on d.LeaderId equals u.Id
                       into tempUD
                       from dd in tempUD.DefaultIfEmpty()
                       //再链父级部门
                       join d2 in _departmentInfoDAL.GetEntities().Where(u => u.IsDelete == false)
        ...
        {
            var parentData = _departmentInfoDAL.GetEntities().Where(it => it.IsDelete == false).Select(d => new
            {
                value = d.Id,
                title = d.DepartmentName,
            });

            var leaderData = _userInfoDAL.GetEntities().Where(it => it.IsDelete == false).Select(d => new
            {
                value = d.Id,
                title = d.UserName,
            });
